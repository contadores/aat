...
/*Estos datos los debemos tomar de base de datos*/
	...
	$rfcEmpresa=' MCE7409273T3'; //Este RFC se debe tomar de la base de datos segun el XLS a acumular.
//Fin
...
/*Obtener excel acumulado del periodo*/
	...
	$inputFileName ='acumulados/RFCEMP2017.xlsx';
	try 
	{
	...
	foreach($_FILES["archivo"]['tmp_name'] as $key => $tmp_name)
	{
		...
		if($_FILES["archivo"]["name"][$key]) {
			...
			if(move_uploaded_file($source, $target_path)) {	
				$inputFileNameSUA ='docs/'.$filename;
			...
			if($rfcEmpresaSUA==$rfcEmpresa) 
			{
				...
				{				
					...
					while ($sua <= count($ultimaFilaSUA)):  //Recorre el EXCEL de SUA 
						...
						$xlsComp =$filaInicial;
						while ($xlsComp <= count($ultimaFila)):   //Recorre el EXCEL de acumulado
							...
							if($NSSXlsComp==$NSSSUA && $BimXlsComp==$bimestreSUA)
							{	
								if ($mesBimestreSUA===1)
								{
									$objPHPExcel->getActiveSheet() //Pone titulo a columna
